################################################################################
#   TITLE   : Spark Advanced Word Count for a local machine or VM that is running
#             spark master server
...
#                   
################################################################################
def Main(sparkMasterURL, inputSamplesDir, outputSamplesDir, sampleFileName):

    # Construct the full path to the sample file
    sampleFullPath = os.path.join(inputSamplesDir, sampleFileName)

    # Construct the full path to the output sample file
    outputSampleFileName = "output-{0}".format(sampleFileName)
    outputSampleFullPath = os.path.join(outputSamplesDir, outputSampleFileName)

    print("The print staments will be outputed on file: {0}".format(outputSampleFullPath))
    f = open(outputSampleFullPath,'w')
    sys.stdout = f

    # Declare Spark Context and Spark Session builder
    sc = pyspark.SparkContext(master = sparkMasterURL, appName = sampleFileName)
    spark = SparkSession.builder.getOrCreate()

    # Read file and add to an RDD object
    rddLines = sc.textFile(sampleFullPath)
    ...
    print("Program finished successfully, check the output file {0}".format(outputSampleFullPath))
...
if __name__ == "__main__":
    ...
    parser.add_argument('sparkMasterURL')
    parser.add_argument('inputSamplesDir')
    parser.add_argument('outputSamplesDir')
    parser.add_argument('sampleFileName')
    args = parser.parse_args()

    Main(args.sparkMasterURL, args.inputSamplesDir, args.outputSamplesDir, args.sampleFileName)
